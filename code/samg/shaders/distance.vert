...
void compute_minimum_distance(in ivec2 Pos,in int Probe, out vec4 Result_color)
{ 
...
// 	if (Vec_data[Pos_min].Value>1) Final_value=1;
// 	else Final_value=0;

//  	Final_value = Vec_data[int(mod(gl_VertexID,165))].Value;
//  	Final_value = float(Pos_min)/165.0;
// 	if (Vec_data[int(mod(gl_VertexID,165))].Value>1) Final_value=1;
// 	else Final_value=0;
