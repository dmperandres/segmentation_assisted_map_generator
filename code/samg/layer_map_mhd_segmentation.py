...
class layer_map_mhd_segmentation:
    ...
    def load_shaders(self):
        if self.shader_program == None:
            with open('shaders/mhd_segmentation.vert', 'r') as file:
                VERTEX_SHADER = file.read()

            with open('shaders/mhd_segmentation.frag', 'r') as file:
    ...
    def set_data(self, normalized_image, segments_image, positions_data, valid_positions, lut, mhd_parameters,
    ...
    def update_layer(self):
        if self.computed == False:
            ...
            glBindVertexArray(self.VAO)
            glUseProgram(self.shader_program);
            glBindImageTexture(0, self.tex_input_normalized_image, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA32F)
            glBindImageTexture(1, self.tex_input_segments_image, 0, GL_FALSE, 0, GL_READ_ONLY, GL_R8)
            glBindImageTexture(2, self.tex_output_image, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32F)
            ...
            # null value
            glUniform1i(13, int(self.null_value))
